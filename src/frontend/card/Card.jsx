...
export default function Card({image, title, isOpen, toggle, description, buttonText, logo}){
    ...
    return (
        <div id="2" className={`pl-10 pr-10 pb-10 bg-blue-100 mb-20 w-3/4 md:w-2/3 ${isOpen ? "max-h-[5000px]" : "lg:max-h-[580px] max-h-[480px]"} transition-all duration-[1000ms] ease-in-out relative overflow-hidden 
        border-2 shadow-lg flex flex-col`}>
                    <img src={image} className="w-full h-[150px] md:h-[200px] lg:h-[280px] mt-5 mb-5 object-cover" />
                    <h1 className="font-bold text-[25px] flex items-center gap-3">{logo} {title}</h1>
